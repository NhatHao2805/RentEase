...
CREATE TABLE USE_SERVICE (
    ...
    ISDELETED           INT DEFAULT 0 COMMENT 'Đánh dấu đã xóa',
    DELETED_DATE        DATE COMMENT 'Ngày bắt đầu xóa (đếm ngược 30 ngày)',
    PRIMARY KEY (TENANTID, SERVICEID, ROOMID)
);
...
ADD CONSTRAINT FK_FEEDBACK_ROOM FOREIGN KEY (ROOMID) REFERENCES ROOM(ROOMID) ON DELETE CASCADE;
...
ADD CONSTRAINT FK_USE_SERVICE_ROOM FOREIGN KEY (ROOMID) REFERENCES ROOM(ROOMID) ON DELETE CASCADE;
