...
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_Contract`(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_Tenant`(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `alter_Contract`(IN contract_id VARCHAR(50))
BEGIN
   SELECT 
      c.CONTRACTID,
		c.ROOMID,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `change_building_key`(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `del_Tenant`(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteDichVu`(IN p_serviceID VARCHAR(10))
BEGIN
    DELETE FROM service WHERE service.SERVICEID = p_serviceID;
END//

CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteTenantService`(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `filter_room`(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetServiceUsage`()
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `INSERT_SERVICE_USAGE`(
    IN P_TENANTID VARCHAR(10),
    IN P_SERVICEID VARCHAR(10),
    IN P_STARTDATE DATE,
    IN P_ENDDATE DATE
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `load_building_by_user`(
    IN usern VARCHAR(20)
)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `load_Contract`(IN p_username VARCHAR(50))
BEGIN
   SELECT 
      c.CONTRACTID,
      c.ROOMID,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `load_floor`(IN p_buildingid VARCHAR(20))
BEGIN
    SELECT NUMOFFLOORS 
    FROM BUILDING 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `load_Room_Extended`(IN p_username VARCHAR(50))
BEGIN
    SELECT 
        r.ROOMID, 
        r.BUILDINGID, 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `load_roomid`(IN p_buildingid VARCHAR(20))
BEGIN
    SELECT ROOMID 
    FROM ROOM 
    WHERE BUILDINGID = p_buildingid;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `load_Tenant`()
BEGIN 
    SELECT * FROM tenant;
END//
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_updateRoom`(
	IN p_roomid VARCHAR(10),
    IN p_buildingid VARCHAR(10),
    IN p_floor INT,
    IN p_type VARCHAR(50),
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_AddParkingArea`(
    IN p_buildingId VARCHAR(10),
    IN p_address VARCHAR(100),
    IN p_type VARCHAR(50),
    IN p_capacity INT,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_DeleteParkingArea`(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_DeleteRoom`(
    IN p_roomid VARCHAR(10),
    OUT p_result INT,
    OUT p_message VARCHAR(255))
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_GetAllBuildings`()
BEGIN
    SELECT BUILDINGID, ADDRESS FROM BUILDING ORDER BY BUILDINGID;
END//

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_GetAllParkingAreas`()
BEGIN
    SELECT * FROM PARKINGAREA ORDER BY AREAID;
END//

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_GetParkingAddresses`()
BEGIN
    SELECT DISTINCT ADDRESS FROM PARKINGAREA ORDER BY ADDRESS;
END//

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_GetParkingAreaCapacity`(
    IN p_areaId VARCHAR(10),
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_GetParkingAreasByBuilding`(
    IN p_buildingId VARCHAR(10)
)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_GetParkingAreasByType`(
    IN p_type VARCHAR(50)
)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_InitParkingAreaForm`(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_UpdateParkingArea`(
    IN p_areaId VARCHAR(10),
    IN p_buildingId VARCHAR(10),
    IN p_address VARCHAR(100),
    IN p_type VARCHAR(50),
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateServicePrice`(
...
CREATE DEFINER=`root`@`localhost` FUNCTION `check_account`(`usern` VARCHAR(50)
) RETURNS int(11)
    READS SQL DATA
    DETERMINISTIC
BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `checkFloorCapacity`(
    p_buildingid VARCHAR(10),
    p_floor INT
) RETURNS int(11)
    DETERMINISTIC
BEGIN
    DECLARE max_rooms_per_floor INT;
    ...
    -- Lấy số phòng tối đa mỗi tầng từ bảng BUILDING
    SELECT NUMOFROOMS/NUMOFFLOORS INTO max_rooms_per_floor
    FROM BUILDING 
    WHERE BUILDINGID = p_buildingid;
    
    -- Trả về TRUE nếu còn chỗ (current_rooms_on_floor < max_rooms_per_floor)
    IF current_rooms_on_floor < max_rooms_per_floor THEN
        RETURN 1;
    END IF;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `createBuildingID`() RETURNS varchar(20) CHARSET utf8mb4 COLLATE utf8mb4_unicode_ci
    DETERMINISTIC
BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `createContractID`() RETURNS varchar(20) CHARSET utf8mb4 COLLATE utf8mb4_unicode_ci
    DETERMINISTIC
BEGIN
    DECLARE max_id VARCHAR(20);
    DECLARE number_part INT;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `createRoomID`(building_id VARCHAR(10), 
    floor_num INT
...
CREATE DEFINER=`root`@`localhost` FUNCTION `createTenantID`() RETURNS varchar(20) CHARSET utf8mb4 COLLATE utf8mb4_unicode_ci
    DETERMINISTIC
BEGIN
    DECLARE max_id VARCHAR(20);
    DECLARE number_part INT;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_GenerateNewBuildingId`() RETURNS varchar(10) CHARSET utf8mb4 COLLATE utf8mb4_unicode_ci
    DETERMINISTIC
BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_GenerateNewParkingAreaId`() RETURNS varchar(10) CHARSET utf8mb4 COLLATE utf8mb4_unicode_ci
    DETERMINISTIC
BEGIN
    DECLARE max_id VARCHAR(10);
    DECLARE number_part INT;
